...
public class Background {
    static final float SCALE_X = 1.5f;
    static final float SCALE_Y = 1.5f;
    static final long FADE_DURATION = 3000;
    static final float BACKDROP_WIDTH = 3000f * SCALE_X;
    static final int color = Color.rgba8888(255, 255, 255, 255);
    ...
    static Biomes drawPrevBack;
    static Biomes drawCurrBack;
    static float drawX;

    static double backgroundScrollX = 0f;
    static double lastPlayerX = 0f;
    ...
    public static void update() {
        ...
        drawX = (float) backgroundScrollX;
        long last = t - swap;
        if (last > FADE_DURATION) {
            last = FADE_DURATION;
        }

        drawAlpha = (int) (last * 255 / FADE_DURATION);
        drawPrevBack = previousBiome;
        drawCurrBack = currentBiome;
    ...
    public static void draw() {
        StackfulRender.pushState(() -> {
            ...
            var bPrev = drawPrevBack;
            var bCurr = drawCurrBack;
            int alphaOld = 255 - drawAlpha;
            if (bPrev != null && alphaOld > 0) {
                drawBackdrop(bPrev.getBackdrop(), Color.withA(color, alphaOld), drawX);
            }
            if (bCurr != null) {
                drawBackdrop(bCurr.getBackdrop(), Color.withA(color, bPrev != null ? drawAlpha : 255), drawX);
            }
        });
    }

    private static void drawBackdrop(Texture backdrop, int color, float startX) {
